...
<template>
    ...
    <div class="flex flex-col flex-grow h-0 p-4 overflow-auto">
        <ul class="space-y-2">
            <li v-bind:class="(row.type === 'user' ? 'flex justify-end' : 'flex justify-start')" v-for="row in messages" :key="row.id">
    ...
    methods: {
        ...
        async loadMessage() {
            try {
                ...
                if (getBots.exists()) {
                    ...
                    updateDoc(doc(db, "Bots", this.id), {
                        seen: true,
                    });

                    const qryMessages = query(collection(db, "Bots/" + this.id + "/Messages"), orderBy('created_at', 'asc'));
        ...
        sendMessage() {
            try {
                ...
                http.post('/dialogflow/detect', data).then((response) => {
